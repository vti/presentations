function Presentation() {
    ...
    presentation.animateSlide = function (n, params) {
        ...
        var div = $('<div style="visibility:hidden">text</div>').appendTo('body');
        div.addClass('slide').addClass(params);
        var margin = $(div).css('margin-left');
        div.remove();

        $(slide).animate({'margin-left': margin}, {complete: function() {
            $(slide).removeClass('far-past past current future far-future');
            $(slide).addClass(params);
        }});
    };
